import pandas as pd
import pytest
from src.extract.extract_playerinfo import (
    extract_playerinfo,
    TYPE,
    FILE_PATH,
...
def mock_log_extract_success(mocker):
    return mocker.patch("src.extract.extract_playerinfo.log_extract_success")


@pytest.fixture
def mock_logger(mocker):
    return mocker.patch("src.extract.extract_playerinfo.logger")


def test_extract_playerinfo_csv_to_dataframe(mocker):
    ...
    mocker.patch(
        "src.extract.extract_playerinfo.pd.read_csv", return_value=mock_df
    )

    df = extract_playerinfo()

    assert isinstance(df, pd.DataFrame)
    pd.testing.assert_frame_equal(df, mock_df)


def test_log_extract_success_playerinfo(
    ...
    mock_end_time = 100.5
    mocker.patch(
        "src.extract.extract_playerinfo.timeit.default_timer",
...
def test_log_playerinfo_error(mocker, mock_logger):
    # Mock pd.read_csv to raise an exception
    mocker.patch(
        "src.extract.extract_playerinfo.pd.read_csv",
