import pandas as pd
import pytest
from src.extract.extract_salaries import (
    extract_salaries,
    TYPE,
    FILE_PATH,
...
def mock_log_extract_success(mocker):
    return mocker.patch("src.extract.extract_salaries.log_extract_success")


@pytest.fixture
def mock_logger(mocker):
    return mocker.patch("src.extract.extract_salaries.logger")


def test_extract_salaries_csv_to_dataframe(mocker):
    ...
    mocker.patch(
        "src.extract.extract_salaries.pd.read_csv", return_value=mock_df
    )
...
def test_log_extract_success_salaries(
    ...
    mock_end_time = 100.5
    mocker.patch(
        "src.extract.extract_salaries.timeit.default_timer",
...
def test_log_salaries_error(mocker, mock_logger):
    # Mock pd.read_csv to raise an exception
    mocker.patch(
        "src.extract.extract_salaries.pd.read_csv",
